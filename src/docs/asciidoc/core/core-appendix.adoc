...
-- that is, not usually beans that are important to an end-info but rather beans that do
...
can use autocompletion to let a info choose between several configuration options
...
This works nicely, but it exposes a lot of Spring plumbing to the end info. What we are
