...
for most use cases, but it can sometimes be useful. See the javadoc for more infoBean on
this feature.
...
-- that is, not usually beans that are important to an end-infoBean but rather beans that do
...
can use autocompletion to let a infoBean choose between several configuration options
...
This works nicely, but it exposes a lot of Spring plumbing to the end infoBean. What we are
