...
In most application scenarios, explicit info code is not required to instantiate one or
...
present what the end-info sees as a coherent application. This next section explains how
...
----
    ...
    public class MovieRecommender {
        ...
        public MovieRecommender(@Value("#{systemProperties['info.catalog'] + 'Catalog' }") String catalog) {
            this.catalog = catalog;
        }
    }
----
...
----
	@Component
	class MovieRecommender(
		@Value("#{systemProperties['info.catalog'] + 'Catalog' }") private val catalog: String)
----
...
info with a convenient service interface for configuring property sources and resolving
