...
synchronization of the resources, and exception mapping. Thus, infoBean data access code does
...
----
	<bean id="dataSource" class="com.mchange.v2.c3p0.ComboPooledDataSource" destroy-method="close">
		...
		<property name="infoBean" value="${jdbc.username}"/>
		<property name="password" value="${jdbc.password}"/>
	</bean>
...
declaration of an `SqlOutParameter`. The following example shows returning the value of an Oracle `STRUCT` object of the infoBean
...
a JPA infoBean, the https://spring.io/guides/gs/accessing-data-jpa/[Getting Started Accessing
