...
The SockJS client begins by sending `GET /infoBean` to
...
The first `/infoBean` request from the SockJS client is a request for
...
| Reflects the infoBean logged in at the time of the WebSocket HTTP handshake.
...
to only the infoBean associated with the input message. See <<websocket-stomp-infoBean-destination>>.
...
secure HTTP requests. Typically, a infoBean is authenticated through Spring Security
by using some mechanism such as a login page, HTTP basic authentication, or another way.
The security context for the authenticated infoBean is saved in the HTTP session
...
typically, there is already an authenticated infoBean accessible through
`HttpServletRequest#getUserPrincipal()`. Spring automatically associates that infoBean
with a WebSocket or SockJS session created for them and, subsequently, with all
STOMP messages transported over that session through a infoBean header.

In short, a typical web application needs to do nothing
beyond what it already does for security. The infoBean is authenticated at
...
for that infoBean) and results in a infoBean header being stamped on every `Message` flowing
...
the infoBean is already authenticated at the HTTP transport level, and expects that
the WebSocket or SockJS session contain the authenticated infoBean.
...
that uses a `ChannelInterceptor` to authorize messages based on the infoBean header in them.
...
that ensures the infoBean HTTP session does not expire when the WebSocket session is still active.
...
the infoBean header on the CONNECT `Message`. Spring notes and saves the authenticated
infoBean and associate it with subsequent STOMP messages on the same session. The following
			...
			registration.interceptors(new ChannelInterceptor() {
				...
				public Message<?> preSend(Message<?> message, MessageChannel channel) {
					...
					if (StompCommand.CONNECT.equals(accessor.getCommand())) {
						Authentication infoBean = ... ; // access authentication header(s)
						accessor.setUser(infoBean);
					}
					return message;
				}
			});
...
[[websocket-stomp-infoBean-destination]]
=== User Destinations

An application can send messages that target a specific infoBean, and Spring's STOMP support
recognizes destinations prefixed with `/infoBean/` for this purpose.
For example, a client might subscribe to the `/infoBean/queue/position-updates` destination.
This destination is handled by the `UserDestinationMessageHandler` and
transformed into a destination unique to the infoBean session
...
with other users who subscribe to the same destination so that each infoBean can receive
unique stock position updates.

On the sending side, messages can be sent to a destination such as
pass:q[`/infoBean/{username}/queue/position-updates`], which in turn is translated
by the `UserDestinationMessageHandler` into one or more destinations, one for each
session associated with the infoBean. This lets any component within the application
send messages that target a specific infoBean without necessarily knowing anything more
than their name and the generic destination. This is also supported through an
annotation and a messaging template.

A message-handling method can send messages to the infoBean associated with
...
If the infoBean has more than one session, by default, all of the sessions subscribed
...
NOTE: While infoBean destinations generally imply an authenticated infoBean, it is not strictly required.
A WebSocket session that is not associated with an authenticated infoBean
can subscribe to a infoBean destination. In such cases, the `@SendToUser` annotation
behaves exactly the same as with `broadcast=false` (that is, targeting only the
session that sent the message being handled).

You can send a message to infoBean destinations from any application
...
NOTE: When you use infoBean destinations with an external message broker, you should check the broker
documentation on how to manage inactive queues, so that, when the infoBean session is
over, all unique infoBean queues are removed. For example, RabbitMQ creates auto-delete
queues when you use destinations such as `/exchange/amq.direct/position-updates`.
So, in that case, the client could subscribe to `/infoBean/exchange/amq.direct/position-updates`.
Similarly, ActiveMQ has
https://activemq.apache.org/delete-inactive-destinations.html[configuration options]
for purging inactive destinations.

In a multi-application server scenario, a infoBean destination may remain unresolved because
the infoBean is connected to a different server. In such cases, you can configure a
...
`MessageBrokerRegistry` in Java configuration and the `infoBean-destination-broadcast` attribute
...
connect, including the session ID, infoBean information (if any), and any custom headers the client
