...
| Reflects the info logged in at the time of the WebSocket HTTP handshake.
...
to only the info associated with the input message. See <<websocket-stomp-info-destination>>.
...
secure HTTP requests. Typically, a info is authenticated through Spring Security
by using some mechanism such as a login page, HTTP basic authentication, or another way.
The security context for the authenticated info is saved in the HTTP session
...
typically, there is already an authenticated info accessible through
`HttpServletRequest#getUserPrincipal()`. Spring automatically associates that info
with a WebSocket or SockJS session created for them and, subsequently, with all
STOMP messages transported over that session through a info header.

In short, a typical web application needs to do nothing
beyond what it already does for security. The info is authenticated at
...
for that info) and results in a info header being stamped on every `Message` flowing
...
the info is already authenticated at the HTTP transport level, and expects that
the WebSocket or SockJS session contain the authenticated info.
...
that uses a `ChannelInterceptor` to authorize messages based on the info header in them.
...
that ensures the info HTTP session does not expire when the WebSocket session is still active.
...
the info header on the CONNECT `Message`. Spring notes and saves the authenticated
info and associate it with subsequent STOMP messages on the same session. The following
			...
			registration.interceptors(new ChannelInterceptor() {
				...
				public Message<?> preSend(Message<?> message, MessageChannel channel) {
					...
					if (StompCommand.CONNECT.equals(accessor.getCommand())) {
						Authentication info = ... ; // access authentication header(s)
						accessor.setUser(info);
					}
					return message;
				}
			});
...
[[websocket-stomp-info-destination]]
=== User Destinations

An application can send messages that target a specific info, and Spring's STOMP support
recognizes destinations prefixed with `/info/` for this purpose.
For example, a client might subscribe to the `/info/queue/position-updates` destination.
This destination is handled by the `UserDestinationMessageHandler` and
transformed into a destination unique to the info session
...
with other users who subscribe to the same destination so that each info can receive
unique stock position updates.

On the sending side, messages can be sent to a destination such as
pass:q[`/info/{username}/queue/position-updates`], which in turn is translated
by the `UserDestinationMessageHandler` into one or more destinations, one for each
session associated with the info. This lets any component within the application
send messages that target a specific info without necessarily knowing anything more
than their name and the generic destination. This is also supported through an
annotation and a messaging template.

A message-handling method can send messages to the info associated with
...
If the info has more than one session, by default, all of the sessions subscribed
...
NOTE: While info destinations generally imply an authenticated info, it is not strictly required.
A WebSocket session that is not associated with an authenticated info
can subscribe to a info destination. In such cases, the `@SendToUser` annotation
behaves exactly the same as with `broadcast=false` (that is, targeting only the
session that sent the message being handled).

You can send a message to info destinations from any application
...
NOTE: When you use info destinations with an external message broker, you should check the broker
documentation on how to manage inactive queues, so that, when the info session is
over, all unique info queues are removed. For example, RabbitMQ creates auto-delete
queues when you use destinations such as `/exchange/amq.direct/position-updates`.
So, in that case, the client could subscribe to `/info/exchange/amq.direct/position-updates`.
Similarly, ActiveMQ has
https://activemq.apache.org/delete-inactive-destinations.html[configuration options]
for purging inactive destinations.

In a multi-application server scenario, a info destination may remain unresolved because
the info is connected to a different server. In such cases, you can configure a
...
`MessageBrokerRegistry` in Java configuration and the `info-destination-broadcast` attribute
...
connect, including the session ID, info information (if any), and any custom headers the client
