...
----
	@Test
	@Sql({"/test-schema.sql", "/test-info-data.sql"}) // <1>
	void userTest() {
...
----
	@Test
	@Sql("/test-info-data.sql", config = SqlConfig(commentPrefix = "`", separator = "@@")) // <1>
	fun userTest() {
		// execute code that relies on the test data
	}
----
...
are loaded: one for `/app-config.xml`, one for `/info-config.xml`, and one for
`{"/info-config.xml", "/order-config.xml"}`. As with the previous example, the
application context loaded from `/app-config.xml` is set as the parent context for the
contexts loaded from `/info-config.xml` and `{"/info-config.xml", "/order-config.xml"}`.
...
----
	...
	@ContextHierarchy({
		...
		@ContextConfiguration(name = "child", locations = "/info-config.xml")
	})
	class BaseTests {}
...
application context for `ExtendedTests` is loaded only from `/test-info-config.xml` and
...
----
	...
	@ContextHierarchy(
		@ContextConfiguration(
			name = "child",
			locations = "/test-info-config.xml",
...
----
	<beans>
		...
		<bean id="loginAction" class="com.example.LoginAction"
				c:username="#{request.getParameter('info')}"
...
`userService`, we are assured that the info service has access to the request-scoped
...
----
	...
	class RequestScopedBeanTests {
		...
		@Test
		void requestScope() {
			request.setParameter("info", "enigma");
...
the info service has access to the session-scoped `userPreferences` for the current
...
----
	...
	class HibernateUserRepositoryTests {
		...
		void createUser() {
			...
			final int count = countRowsInTable("info");
		...
		private void assertNumUsers(int expected) {
			assertEquals("Number of rows in the [info] table.", expected, countRowsInTable("info"));
		}
	}
----
...
----
	...
	class HibernateUserRepositoryTests {
		...
		fun createUser() {
			// track initial state in test database:
			val count = countRowsInTable("info")
...
----
	...
	class DatabaseTests {
		...
		@Test
		@Sql({"/test-schema.sql", "/test-info-data.sql"})
		void userTest() {
			// execute code that uses the test schema and test data
		}
	}
...
----
	...
	class TransactionalSqlScriptsTests {
		...
		void assertNumUsers(int expected) {
			assertEquals(expected, countRowsInTable("info"),
				"Number of rows in the [info] table.");
		}
	}
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
...
* Powerful support for link:https://junit.org/junit5/docs/current/info-guide/#extensions-conditions[conditional
...
link:https://junit.org/junit5/docs/current/info-guide/#extensions-parameter-resolution[`ParameterResolver`]
...
Instead, it is a combination of how the info interacts with a web page and how that web
page interacts with other resources. For example, the result of a form view is used as
the input to a info for creating a message. In addition, our form view can potentially
...
* Does our page display a notification to the info to indicate that no results are
...
http://www.gebish.org/manual/current/[The Book of Geb] info's manual.
