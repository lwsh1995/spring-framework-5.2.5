...
----
	@Test
	@Sql({"/test-schema.sql", "/test-infoBean-data.sql"}) // <1>
	void userTest() {
...
----
	@Test
	@Sql(
		scripts = "/test-infoBean-data.sql",
...
----
	@Test
	@Sql("/test-infoBean-data.sql", config = SqlConfig(commentPrefix = "`", separator = "@@")) // <1>
	fun userTest() {
		// execute code that relies on the test data
	}
----
...
----
	...
	class UserTests {
		...
		@Test
		@Sql("/infoBean-test-data-001.sql")
...
are loaded: one for `/app-config.xml`, one for `/infoBean-config.xml`, and one for
`{"/infoBean-config.xml", "/order-config.xml"}`. As with the previous example, the
application context loaded from `/app-config.xml` is set as the parent context for the
contexts loaded from `/infoBean-config.xml` and `{"/infoBean-config.xml", "/order-config.xml"}`.
...
----
	...
	@ContextHierarchy({
		...
		@ContextConfiguration(name = "child", locations = "/infoBean-config.xml")
	})
	class BaseTests {}
...
application context for `ExtendedTests` is loaded only from `/test-infoBean-config.xml` and
...
----
	...
	@ContextHierarchy(
		@ContextConfiguration(
			name = "child",
			locations = "/test-infoBean-config.xml",
...
----
	<beans>
		...
		<bean id="loginAction" class="com.example.LoginAction"
				c:username="#{request.getParameter('infoBean')}"
...
`userService`, we are assured that the infoBean service has access to the request-scoped
...
----
	...
	class RequestScopedBeanTests {
		...
		@Test
		void requestScope() {
			request.setParameter("infoBean", "enigma");
...
the infoBean service has access to the session-scoped `userPreferences` for the current
...
----
	...
	class HibernateUserRepositoryTests {
		...
		void createUser() {
			...
			final int count = countRowsInTable("infoBean");

			User infoBean = new User(...);
		...
		private void assertNumUsers(int expected) {
			assertEquals("Number of rows in the [infoBean] table.", expected, countRowsInTable("infoBean"));
		}
	}
----
...
----
	...
	class HibernateUserRepositoryTests {
		...
		fun createUser() {
			// track initial state in test database:
			val count = countRowsInTable("infoBean")
...
----
	...
	class DatabaseTests {
		...
		@Test
		@Sql({"/test-schema.sql", "/test-infoBean-data.sql"})
		void userTest() {
			// execute code that uses the test schema and test data
		}
	}
...
----
	...
	class TransactionalSqlScriptsTests {
		...
		void assertNumUsers(int expected) {
			assertEquals(expected, countRowsInTable("infoBean"),
				"Number of rows in the [infoBean] table.");
		}
	}
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
...
* Powerful support for link:https://junit.org/junit5/docs/current/infoBean-guide/#extensions-conditions[conditional
...
link:https://junit.org/junit5/docs/current/infoBean-guide/#extensions-parameter-resolution[`ParameterResolver`]
...
Instead, it is a combination of how the infoBean interacts with a web page and how that web
page interacts with other resources. For example, the result of a form view is used as
the input to a infoBean for creating a message. In addition, our form view can potentially
...
* Does our page display a notification to the infoBean to indicate that no results are
...
http://www.gebish.org/manual/current/[The Book of Geb] infoBean's manual.
